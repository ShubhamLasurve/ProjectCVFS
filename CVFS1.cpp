...
void man(char *name)
{
    ...
    {
        printf("Description : Used to create new reglar file\n");
...
void DisplayHelp()
{
    ...
    printf("read : To read the contents of the files\n");
...
int WriteFile(int fd, char *arr, int isize)
{
    ...
    strncpy(arr,(UFDTArr[fd].ptrfiletable->ptrinode->Buffer) + (UFDTArr[fd].ptrfiletable->writeoffset),isize);
...
int LseekFile(int fd,int size,int from)
{
    ...
    else if(UFDTArr[fd].ptrfiletable->mode == WRITE)
    {
        if(from == CURRENT)
        {
            ...
            (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) = (UFDTArr->ptrfiletable->writeoffset) + size;
...
int main()
{
    ...
    while(1)
    {
        ...
        fgets(str,80,stdin);        //Either we can use scanf("%[^'\n']s",str);
        ...
        if(count == 1)
        {
            ...
            {
                printf("Terminating the Shubham Virtual File System\n");
                break;
            }
            else
            {
